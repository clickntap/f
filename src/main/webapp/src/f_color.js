...
(function () {
  function fcolor(color, elmnt) {
    if (!(this instanceof fcolor)) { return new fcolor(color, elmnt); }
...
function fSetColorsByAttribute() {
  var z, i, att;
  z = document.getElementsByTagName("*");
  for (i = 0; i < z.length; i++) {
    att = z[i].getAttribute("data-f-color");
    if (att) {
      z[i].style.backgroundColor = fcolor(att).toRgbString();
    }
  }
}
